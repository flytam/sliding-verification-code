...
var SlidingVerification =
/*#__PURE__*/
function (_React$Component) {
  _inherits(SlidingVerification, _React$Component);

  function SlidingVerification() {
    var _getPrototypeOf2;

    var _this;

    _classCallCheck(this, SlidingVerification);

    for (var _len = arguments.length, args = new Array(_len), _key = 0; _key < _len; _key++) {
      args[_key] = arguments[_key];
    }

    _this = _possibleConstructorReturn(this, (_getPrototypeOf2 = _getPrototypeOf(SlidingVerification)).call.apply(_getPrototypeOf2, [this].concat(args)));
    ...
    _defineProperty(_assertThisInitialized(_assertThisInitialized(_this)), "generateImg", function () {
      // 整张图片
      var context = _this.canvas.getContext("2d"); // 缺块原图


      var dirtyContext = _this.dirtyCanvas.getContext("2d");

      var _this$props = _this.props,
          src = _this$props.src,
          innerLengthPencent = _this$props.innerLengthPencent;
      var img = new Image();
      img.crossOrigin = "anonymous";
      img.src = src;

      img.onload = function () {
        _this.innerLength = Math.min(innerLengthPencent, 0.2) * img.width; // 缺块偏移位置计算

        var innerX = _this.innerLength + (img.width - 3 * _this.innerLength) * Math.random();
        _this.innerX = innerX;
        var innerY = _this.innerLength + (img.height - 3 * _this.innerLength) * Math.random();

        _this.setState({
          width: img.width,
          height: img.height,
          slideStyle: {
            top: "-".concat(img.height - _this.innerLength - innerY, "px")
          }
        });

        context.drawImage(img, 0, 0);
        var imageData = context.getImageData(innerX, innerY, _this.innerLength, _this.innerLength); //

        dirtyContext.putImageData(imageData, 0, 0);
        var coverCanvas = document.createElement("canvas");
        coverCanvas.width = _this.innerLength;
        coverCanvas.height = _this.innerLength;
        var coverContext = coverCanvas.getContext("2d");
        context.fillStyle = "#000000";
        coverContext.fillRect(0, 0, _this.innerLength, _this.innerLength);
        var coverImageData = coverContext.getImageData(0, 0, _this.innerLength, _this.innerLength);
      ...
      document.addEventListener("mousemove", function (e) {
        if (_this.isDown) {
          ...
          _this.setState(function (preState) {
            return {
              slideStyle: _objectSpread({}, preState.slideStyle, {
                left: "".concat(currentX - _this.initialX, "px")
              })
            };
          });

          if (typeof onSlide === "function") {
            onSlide(Math.abs(_this.innerLength + _this.innerX - (currentX - _this.initialX)));
          }
        }
      });
      document.addEventListener("mouseup", function (e) {
        if (_this.isDown) {
          var _this$props2 = _this.props,
              ...
              errorRange = _this$props2.errorRange;
          var currentX = e.pageX; // 检测是否到了

          _this.isDown = false;
          var isOK = false;

          if (typeof onFinish === "function") {
            if (Math.abs(_this.innerLength + _this.innerX - (currentX - _this.initialX)) <= errorRange) {
              isOK = true;
            }
    ...
    _defineProperty(_assertThisInitialized(_assertThisInitialized(_this)), "isDown", false);

    _defineProperty(_assertThisInitialized(_assertThisInitialized(_this)), "initialX", 0);
    ...
    _defineProperty(_assertThisInitialized(_assertThisInitialized(_this)), "render", function () {
      var canvasClass = _this.props.canvasClass;
      var _this$state = _this.state,
          width = _this$state.width,
          height = _this$state.height,
          slideStyle = _this$state.slideStyle;
      return react.createElement("div", null, react.createElement("canvas", {
        ref: function ref(node) {
          return _this.dirtyCanvas = node;
        },
        width: _this.innerLength || 0,
        height: _this.innerLength || 0,
        style: _objectSpread({
          position: "relative"
        }, slideStyle)
...
_defineProperty(SlidingVerification, "defaultProps", {
  src: "",
  errorRange: 10,
  innerLengthPencent: 0.2,
  // 内部缺块正方形的边长百分比  0-0.2
